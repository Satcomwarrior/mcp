...
export const getEthBalance: Tool = {
  ...
  handle: async (context: Context, params) => {
    ...
    const usdSet = new Set<string>();

    for (const pattern of balancePatterns) {
      for (const match of snapshotText.matchAll(pattern)) {
        balanceSet.add(match[0]);
        if (balanceSet.size >= 10) break;
      }
      if (balanceSet.size >= 10) break;
    }

    if (includeTokens && balanceSet.size < 10) {
    ...
    for (const pattern of usdPatterns) {
      ...
      if (usdSet.size >= 5) break;
    }

    const uniqueBalances = Array.from(balanceSet).slice(0, 10);
    const uniqueUsdValues = Array.from(usdSet).slice(0, 5);
