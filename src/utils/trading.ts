...
// Cache for Intl.NumberFormat instances to avoid expensive repeated instantiation overhead (~60x speedup)
const numberFormatCache = new Map<string, Intl.NumberFormat>();
const MAX_FORMATTER_CACHE_SIZE = 1000;
...
export function formatPrice(price: number, currency = "USD"): string {
  ...
  if (!formatter) {
    ...
    // Prevent unbounded memory growth if flooded with untrusted/random currency codes
    if (numberFormatCache.size >= MAX_FORMATTER_CACHE_SIZE) {
