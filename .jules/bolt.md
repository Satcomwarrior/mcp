...
## 2026-02-23 - Cache Intl.NumberFormat instantiation
**Learning:** Instantiating `Intl.NumberFormat` repeatedly in a hot path like `formatPrice` is a significant performance bottleneck (e.g., ~7000ms vs ~80ms for 100k iterations).
**Action:** Always cache `Intl.NumberFormat` instances using a `Map` keyed by configuration options (e.g., currency and decimal places) to achieve substantial speed improvements.
