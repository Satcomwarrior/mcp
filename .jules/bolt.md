...
## 2025-05-04 - Intl.NumberFormat Instantiation Bottleneck
**Learning:** Instantiating `Intl.NumberFormat` inside high-frequency functions (like `formatPrice` in `src/utils/trading.ts`) creates a massive performance bottleneck, increasing execution time by up to 66x.
**Action:** Always cache and reuse `Intl.NumberFormat` instances using a `Map` keyed by configuration options (e.g., currency, maxDecimals) rather than creating new ones on every function call.
