...
**Learning:** Repeatedly instantiating `Intl.NumberFormat` is extremely expensive (~15.3 seconds for 100k iterations). Caching instances with a module-level `Map` speeds this up by ~70x (down to ~216ms).
**Action:** Always cache `Intl.NumberFormat` (and similar `Intl` objects) when used in hot paths or loops. Implement a max cache size to prevent memory leaks from untrusted inputs.
