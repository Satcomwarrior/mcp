...
## 2025-04-25 - Fast-path Regex Filtering
**Learning:** Parsing large text snapshots line-by-line and applying string manipulation (`toLowerCase()`) on every line is a significant bottleneck.
**Action:** Introduce a module-level pre-compiled regex (e.g., `/gwei/i`) and use `RegExp.test()` as a fast-path filter *before* performing expensive string manipulations or array regex matches, yielding ~9x speedups.
