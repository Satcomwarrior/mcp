...
## 2026-06-13 - Lazy Regex Iteration on ARIA Snapshots
**Learning:** String.prototype.match(/g/) evaluates the entire text even if only a few matches are needed, causing massive overhead on large texts like ARIA snapshots.
**Action:** Use String.prototype.matchAll() with an early break and explicitly safely-enforced global flags instead of match() and slice() to avoid full-document regex evaluation when extracting limited data.
